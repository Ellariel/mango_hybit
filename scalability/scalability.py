...
for i in tqdm(cells_count):
    ...
    for j in tqdm(hierarchy, desc=f"cells_count: {i}", leave=False):
        for _ in tqdm(range(attempts), desc=f"hierarchy: {j}", leave=False):
            ...
            os.system(f"python scenario.py --verbose -1 --seed {seed} --dir {base_dir} --output_file {output_filename} --clean False --performance True --cells {i} --hierarchy {j} > {NULL if not stdout_logs else os.path.join(logs_dir, f'stdout_{i}_{j}_{seed}.log')} 2>&1")
