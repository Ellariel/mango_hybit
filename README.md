...
### COHDA example scenario

 Within the COHDA scenario, a simple *pandapower* network with flexible power units logically organized into two cells can be examined. The optimization algorithm can be specified using MAS_CONFIG, and the algorithm *between-cells** and *within-cell* can be either *default*, or *cohda*. The *default* algorithm assumes altruistic behavior and infinite power of the external network, while the *cohda* algorithm is based on [COHDA](https://www.inderscience.com/offers.php?id=85895) and is able to solve the power flow optimization problem under more complex conditions.

1) To run the experiments one should go to the `examples/cohda` folder and  install the required packages:
...
4) Check out the results in `results` folder. There is an also analysis notebook `cohda_analysis.ipynb`

### Scalablility example scenario

**Note that this old example script can be run under an older version of the library, available at [2f586f0f](https://gitlab.com/hybit1/massca/-/tree/2f586f0fb13b01bf08bf78ba7931a633fde00c3e)**
...
* COHDA implementation has been adapted and [forked](https://gitlab.com/mosaik/examples/mosaik-cohda) from the [ZDIN-ZLE project](https://gitlab.com/zdin-zle/models/mosaik-cohda). See also, [paper one](https://www.inderscience.com/offers.php?id=85895) and [paper two](https://uol.de/f/2/dept/informatik/ag/ui/publications/HLS14b.pdf).
